	...
	while ($row = mysqli_fetch_array($scheds)) {
		...
		if(($_start_time >= $row['start_time'] AND $_start_time < $row['end_time']) 
			...
			if(array_key_exists($row['sched_id'], $_SESSION['sched_ids'] )){
				...
				return '<div style="max-height: 10px; height: 10px; overflow: visible; z-index: 1000px;">Teacher: <strong>'.$member['member_first'].' '.$member['member_last'].'</strong>; Class: <strong>'.$row['cys'].'</strong>; Subject: <strong>'.$row['subject_code'].'</strong>; Units: <strong>'.$sub['subject_units'].'</strong>; Room: <strong>'.$row['room'].'</strong></div>';
			}
		}
	}
} 
